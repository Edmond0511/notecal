-- Migration script to remove ai_provider references
-- Run this in the Supabase SQL Editor

-- Remove ai_provider column from profiles table if it exists
DO $$
BEGIN
    IF EXISTS (
...
-- Remove ai_provider column from nutrition_cache table if it exists
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns
...
-- Remove ai_provider column from api_usage table if it exists
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns
...
-- Remove provider column from ai_prompts table if it exists
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns
...
-- Update the existing ai_prompts record if it still has gemini as provider
...
-- Drop and recreate the popular_foods view without ai_provider
...
RAISE NOTICE 'Migration completed: Successfully removed all ai_provider references';